/* ── AgentFeed v3.0 — Live scrolling feed of agent decisions with robust flex layout ── */
...
export default function AgentFeed({ onSelect }: AgentFeedProps) {
  ...
  return (
    <div className="bg-[#101623] border border-[#222F46] rounded-[12px] h-[520px] flex flex-col p-0 overflow-hidden shadow-xl">
      {/* Feed Header */}
      <div className="p-3.5 px-4 border-b border-[#1A2538] bg-[#090D16]/80 flex items-center justify-between shrink-0">
      ...
      >
        {entries.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center p-8 text-center gap-2">
            ...
            <p className="text-[#566782] text-[11px] font-mono max-w-[340px]">
              Execute a batch or trigger Failure Studio to watch autonomous recoveries stream in real-time.
            </p>
          </div>
        ) : (
            ...
            {entries.map((entry) => {
              ...
              return (
                <motion.div
                  ...
                  className={`cursor-pointer p-3.5 hover:bg-[#182234] transition-all group ${
                    isRecovered ? 'bg-[#10B981]/[0.03]' : ''
                  }`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="font-mono text-[12px] font-bold text-[#F0F6FC] group-hover:text-[#38BDF8] transition-colors">
                        {entry.payment_id}
                      </span>
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
                      ...
                      <span className="text-[10px] bg-[#182234] text-[#38BDF8] border border-[#38BDF8]/30 px-1.5 py-0.5 rounded-[4px] font-mono flex items-center gap-1 group-hover:bg-[#38BDF8] group-hover:text-white transition-all">
                        <Smartphone size={10} />
                        <span>Inspect</span>
                      </span>
                    </div>
                  ...
                  {entry.llm_reasoning && (
                    <div className="mt-2 text-[12px] text-[#94A3B8] italic border-l-2 border-[#38BDF8] pl-2.5 bg-[#38BDF8]/5 py-1 rounded-[0_4px_4px_0] line-clamp-2">
                      &quot;{entry.llm_reasoning}&quot;
                    </div>
                  )}
                </motion.div>
